<script lang="ts">
	...
	import { PedidosController } from "$lib/controllers/pedidos.controller";
    ...
    const headers = [
        ...
        { label:'Data do Pedido', colSpan:'col-span-2' },
        { label:'Valor Total', colSpan:'col-span-1' },
        { label:'Valor Status', colSpan:'col-span-1' },
        { label:'Ações', colSpan:'col-span-1' }
    ]

    let openFilters = $state(false)
    let pedidos:any[] = $state([])

    onMount(async() => {
        const [res, err] = await pedidosController.listarPedidos(1)
        if(err) return toast.error('Erro ao listar pedidos', err)
        console.log(res)
        pedidos = res.data.results
    })
...
{#snippet header()}
    <div class="flex justify-between items-center w-full">
        <h1 class="font-bold">Pedidos</h1>
        <span class="flex gap-3 items-center">
            <PrintButton/>
            <MainButton action={() => goto('/novo-pedido')} label="Novo pedido"/>
        </span>
    </div>
{/snippet}
...
{#snippet tabela()}
    {#each headers as _, i}
        <div class="w-full px-3 py-1.5 text-sm font-light bg-white text-sub-600 grid grid-cols-8 last:rounded-b-lg gap-8 not-last:border-b border-[#25384B]">
...
{@render filtros()}
{#if pedidos.length > 0}
    <Tabela {headers} children={tabela} gridCols="grid-cols-8"/>
{:else}
    <EmptyState>
        <h3 class="font-bold text-2xl">Nenhum pedido encontrado</h3>
    </EmptyState>
{/if}
